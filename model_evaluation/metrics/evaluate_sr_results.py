...
def CalLPIPS(SRFolder,GTFolder):
    ...
    for i in nameList:
        ...
        imageA = Image.open(imageA).convert('RGB')
        imageB = Image.open(imageB).convert('RGB')
        image_width = (imageA.width // 4) * 4
        image_height = (imageA.height // 4) * 4
        imageA = imageA.resize((image_width, image_height), resample=Image.BICUBIC)
        image_width = (imageB.width // 4) * 4
        image_height = (imageB.height // 4) * 4
        imageB = imageB.resize((image_width, image_height), resample=Image.BICUBIC)
        
        imageA = np.array(imageA)
        imageB = np.array(imageB)
        imageA = torch.Tensor((imageA / 127.5 - 1.0)[:, :, :, np.newaxis].transpose((3, 2, 0, 1)))
        imageB = torch.Tensor((imageB / 127.5 - 1.0)[:, :, :, np.newaxis].transpose((3, 2, 0, 1)))
